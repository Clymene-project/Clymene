...
func StreamWriteYAMLResponse(w http.ResponseWriter, iter chan interface{}, logger *zap.Logger) {
	w.Header().Set("Content-Type", "application/yaml")
	for v := range iter {
		data, err := yaml.Marshal(v)
		if err != nil {
			logger.Error("yaml marshal failed", zap.Error(err))
			continue
		}
		_, err = w.Write(data)
		if err != nil {
			logger.Error("write http response failed", zap.Error(err))
			return
		}
	}
}
