...
func NewZookeeperTreeCache(conn *zk.Conn, path string, events chan ZookeeperTreeCacheEvent, logger *zap.Logger) *ZookeeperTreeCache {
...
func (tc *ZookeeperTreeCache) loop(path string) {
	...
	if err != nil {
		tc.logger.Error("Error during initial read of Zookeeper", zap.Error(err))
		failure()
	}
	...
	for {
		...
		case ev := <-tc.head.events:
			tc.logger.Debug("Received Zookeeper event",
				zap.String("event.Server", ev.Server),
				zap.String("event.Path", ev.Path),
				zap.String("event.State", ev.State.String()),
				zap.String("event.Type", ev.Type.String()))
			if failureMode {
				continue
			}
			...
			if ev.Type == zk.EventNotWatching {
				...
				if err != nil {
					tc.logger.Error("Error during processing of Zookeeper event", zap.Error(err))
					failure()
				} else if tc.head.data == nil {
					tc.logger.Error("Error during processing of Zookeeper event",
						zap.String("err", "path no longer exists"), zap.String("path", tc.prefix))
					failure()
				}
			}
		case <-retryChan:
			...
			if err := tc.recursiveNodeUpdate(tc.prefix, tc.head); err != nil {
				tc.logger.Error("Error during Zookeeper resync", zap.Error(err))
