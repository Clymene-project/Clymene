...
import (
	...
	"github.com/bourbonkk/Clymene/pkg/recoveryhandler"
...
const (
	healthCheckHTTPPort = "health-check-http-ports"
	adminHTTPPort       = "admin-http-ports"
	adminHTTPHostPort   = "admin.http.host-ports"
)
...
func (s *AdminServer) AddFlags(flagSet *flag.FlagSet) {
	flagSet.String(adminHTTPHostPort, s.adminHostPort, fmt.Sprintf("The host:ports (e.g. 127.0.0.1%s or %s) for the admin server, including health check, /metrics, etc.", s.adminHostPort, s.adminHostPort))
}
...
func (s *AdminServer) Serve() error {
	...
	s.logger.Info(
		...
		zap.String("http.host-ports", l.Addr().String()),
