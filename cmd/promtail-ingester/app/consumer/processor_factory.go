...
import (
	"github.com/Clymene-project/Clymene/cmd/promtail-ingester/app/consumer/offset"
	"github.com/Clymene-project/Clymene/cmd/promtail-ingester/app/processor"
	"github.com/Clymene-project/Clymene/cmd/promtail-ingester/app/processor/decorator"
...
type ProcessorFactoryParams struct {
	Parallelism    int
	Topic          string
	BaseProcessor  processor.LogProcessor
...
func (c *ProcessorFactory) new(partition int32, minOffset int64) processor.LogProcessor {
	...
	logProcessor := processor.NewDecoratedProcessor(c.metricsFactory, cp)
	pp := processor.NewParallelProcessor(logProcessor, c.parallelism, c.logger)
...
func newStartedProcessor(parallelProcessor startProcessor, services ...service) processor.LogProcessor {
	s := &startedProcessor{
		services:  services,
