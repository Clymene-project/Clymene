...
// with jitter.
func NewRetryingProcessor(f metrics.Factory, processor processor.LogProcessor, opts ...RetryOption) processor.LogProcessor {
	options := defaultOpts
	for _, opt := range opts {
		opt(&options)
	}

	m := f.Namespace(metrics.NSOptions{Name: "log-processor", Tags: nil})
