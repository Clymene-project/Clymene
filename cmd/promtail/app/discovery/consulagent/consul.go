...
func (d *Discovery) getDatacenter() error {
	...
	if err != nil {
		d.logger.Error("Error retrieving datacenter name", zap.Error(err))
...
func (d *Discovery) watchServices(ctx context.Context, ch chan<- []*targetgroup.Group, services map[string]func()) {
	...
	d.logger.Debug("Watching services", zap.String("tags", strings.Join(d.watchedTags, ",")))
	...
	if err != nil {
		d.logger.Error("Error refreshing service list", zap.Error(err))
		rpcFailuresCount.Inc()
		time.Sleep(retryInterval)
		return
	}
...
func (srv *consulService) watch(ctx context.Context, ch chan<- []*targetgroup.Group, agent *consul.Agent) {
	srv.logger.Debug("Watching service", zap.String("service", srv.name), zap.String("tags", strings.Join(srv.tags, ",")))
	...
	if err != nil {
		srv.logger.Error("Error refreshing service", zap.String("service", srv.name), zap.String("tags", strings.Join(srv.tags, ",")), zap.Error(err))
		rpcFailuresCount.Inc()
		time.Sleep(retryInterval)
		return
	}
	...
	self, err := agent.Self()
	if err != nil {
		srv.logger.Error("failed to get agent info from agent api", zap.Error(err))
		return
	}
	var member = consul.AgentMember{}
	memberBytes, err := json.Marshal(self["Member"])
	if err != nil {
		srv.logger.Error("failed to get member information from agent", zap.Error(err))
		return
	}
	err = json.Unmarshal(memberBytes, &member)
	if err != nil {
		srv.logger.Error("failed to unmarshal member information from agent", zap.Error(err))
		return
	}
