...
func NewEC2Discovery(conf *EC2SDConfig, logger *zap.Logger) *EC2Discovery {
	if logger == nil {
		logger = zap.NewNop()
	}
	d := &EC2Discovery{
...
func (d *EC2Discovery) refresh(ctx context.Context) ([]*targetgroup.Group, error) {
	...
	if d.azToAZID == nil {
		if err := d.refreshAZIDs(ctx); err != nil {
			d.logger.Error("msg Unable to describe availability zones", zap.Error(err))
			...
			for _, inst := range r.Instances {
				...
				if !ok && d.azToAZID != nil {
					d.logger.Debug(
						...
						zap.String("az", *inst.Placement.AvailabilityZone))
				}
