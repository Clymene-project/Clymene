...
func NewDiscovery(cfg *SDConfig, logger *zap.Logger) *Discovery {
	if logger == nil {
		logger = zap.NewNop()
	}
	d := &Discovery{
...
func newAzureResourceFromID(id string, logger *zap.Logger) (azureResource, error) {
		...
		go func(i int, vm virtualMachine) {
			...
			for _, nicID := range vm.NetworkInterfaces {
				...
				if networkInterface.Primary == nil {
					d.logger.Debug("Skipping deallocated virtual machine", zap.String("machine", vm.Name))
					return
				}
