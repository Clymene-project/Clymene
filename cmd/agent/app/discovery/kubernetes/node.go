...
func NewNode(l *zap.Logger, inf cache.SharedInformer) *Node {
	if l == nil {
		l = zap.NewNop()
	}
...
func (n *Node) process(ctx context.Context, ch chan<- []*targetgroup.Group) bool {
	...
	if err != nil {
		n.logger.Error("converting to Node object failed", zap.Error(err))
		return true
	}
...
func (n *Node) buildNode(node *apiv1.Node) *targetgroup.Group {
	...
	if err != nil {
		n.logger.Warn("No node address found", zap.Error(err))
		return nil
	}
