...
type Discovery struct {
	*refresh.Discovery
	...
	lookupFn func(name string, qtype uint16, logger *zap.Logger) (*dns.Msg, error)
}
	...
	for _, name := range d.names {
		go func(n string) {
			if err := d.refreshOne(ctx, n, ch); err != nil && err != context.Canceled {
				d.logger.Error("Error refreshing DNS targets", zap.Error(err))
			}
			wg.Done()
		}(name)
	}
...
func (d *Discovery) refreshOne(ctx context.Context, name string, ch chan<- *targetgroup.Group) error {
	...
	for _, record := range response.Answer {
		...
		default:
			d.logger.Warn("Invalid SRV record", zap.String("record", record.String()))
			continue
		}
...
// nightmare.
func lookupWithSearchPath(name string, qtype uint16, logger *zap.Logger) (*dns.Msg, error) {
...
func lookupFromAnyServer(name string, qtype uint16, conf *dns.ClientConfig, logger *zap.Logger) (*dns.Msg, error) {
	client := &dns.Client{}
	...
	for _, server := range conf.Servers {
		...
		if err != nil {
			logger.Warn("DNS resolution failed", zap.String("server", server), zap.String("name", name), zap.Error(err))
			continue
		}
